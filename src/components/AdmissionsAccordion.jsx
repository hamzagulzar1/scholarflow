				...
				return (
					<React.Fragment key={level}>
						<h2 id={`accordion-color-heading-${index + 1}`}>
							<button
								type="button"
								className="flex items-center justify-between w-full p-5 font-medium text-gray-500 border border-black rounded  dark:text-gray-400 gap-3"
								onClick={() => toggleAccordion(level)}
								aria-expanded={isOpen}
								aria-controls={`accordion-color-body-${index + 1}`}
							>
								<span className="text-black">{level} Programs</span>
								<svg
									data-accordion-icon
						...
						>
							<div className="p-5 border border-b-2 border-gray-200 dark:border-gray-700 dark:bg-gray-900">
								{levelData ? (
									Object.entries(levelData.updates).map(([key, value]) => (
										<p key={key} className="mb-2 text-white">
											{key}: {value}
										</p>
									))
								) : (
